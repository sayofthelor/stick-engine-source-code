...
class FunkinLua {
	...
	public function new(script:String) {
		...
		// shader shit
		Lua_helper.add_callback(lua, "initLuaShader", function(name:String, type:String = "flixel", glslVersion:Int = 120) {
			if(!ClientPrefs.shaders) return false;

			#if (!flash && MODS_ALLOWED && sys)
			return initLuaShader(name, type, glslVersion);
			#else
		...
		Lua_helper.add_callback(lua, "setSpriteShader", function(obj:String, shader:String) {
			...
			if(leObj != null) {
				...
				else if (arr[2] == "shadertoy") {
					var s = new FlxShaderToyRuntimeShader(arr[0], leObj.width, leObj.height);
					leObj.shader = s;
					PlayState.instance.shaderFixThing.set(obj, s);
				}
				return true;
			}
			#else
		...
		Lua_helper.add_callback(lua, "removeSpriteShader", function(obj:String) {
			...
			if(leObj != null) {
				if (PlayState.instance.shaderFixThing.exists(obj)) {
					PlayState.instance.shaderFixThing.remove(obj);
				}
				leObj.shader = null;
				return true;
			}
		...
		Lua_helper.add_callback(lua, "changeIcon", function(player:Int, icon:String) {
			var iconToChange:Dynamic = null;
			if (Paths.fileExists("images/icons/icon-" + icon, IMAGE)) iconToChange = Paths.image("images/icons/icon-" + icon) else if (Paths.fileExists("images/icons/" + icon, IMAGE)) iconToChange = Paths.image("images/icons/"+icon);
			if (iconToChange != null)
				{
					switch (player)
					{
						case 0:
							PlayState.instance.iconP1.changeIcon(iconToChange);
						case 1:
							PlayState.instance.iconP2.changeIcon(iconToChange);
					}
				}
		});
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(PlayState.instance.modchartSprites.exists(tag) && !ClientPrefs.optimization) {
	...
	function initLuaShader(name:String, type:String = "flixel", ?glslVersion:Int = 120)
	{
		if(!ClientPrefs.shaders) return false;
		...
		for (folder in foldersToCheck)
		{
			if(FileSystem.exists(folder))
			{
				...
				if(found)
				{
					PlayState.instance.runtimeShaders.set(name, [frag, vert, type]);
...
class ModchartText extends FlxText
{
	...
	public function new(x:Float, y:Float, text:String, width:Float)
	{
		...
		setFormat(Paths.font("PhantomMuff.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
class DebugLuaText extends FlxText
{
	...
	public function new(text:String, parentGroup:FlxTypedGroup<DebugLuaText>, color:FlxColor) {
		...
		setFormat(Paths.font("PhantomMuff.ttf"), 20, color, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scrollFactor.set();
		borderSize = 1;
	}
