...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		scoreTxt.setFormat(Paths.font("PhantomMuff.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		notFinalTxt.setFormat(Paths.font("PhantomMuff.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function leCoolSongName(durationIn:Float = 1, durationOut:Float = 1){
		var songName:String = Paths.formatToSongPath(SONG.song);
		var leBox:FlxSprite = new FlxSprite(-550, 300).loadGraphic(Paths.image('SongName'));
		leBox.scrollFactor.set();
		leBox.cameras = [camHUD];
		add(leBox);
		var leSongName:FlxText = new FlxText(-800, 320, 0, songName, 60);
		leSongName.scrollFactor.set();
		leSongName.cameras = [camHUD];
		leSongName.setFormat(Paths.font("PhantomMuff.ttf"), 60, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(leSongName);
		new FlxTimer().start(1, function(tmr:FlxTimer)
			{
		FlxTween.tween(leBox, {x: -100}, durationIn, {type:PERSIST, ease:FlxEase.backInOut});
		FlxTween.tween(leSongName, {x: 50}, durationIn, {type:PERSIST, ease:FlxEase.backInOut});
			});
		new FlxTimer().start(6, function(tmr:FlxTimer)
			{
		FlxTween.tween(leSongName, {x: -800}, durationOut, {type:PERSIST, ease:FlxEase.backInOut});
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		noteDiffText.setFormat(Paths.font("PhantomMuff.ttf"), 32, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
